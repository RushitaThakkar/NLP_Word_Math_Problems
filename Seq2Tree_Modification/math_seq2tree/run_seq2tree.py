...
from src.gen_bert_embeddings import *
...
for fold in range(5):
    ...
    with open("input_vocab_dict.json", "w") as f:
          json.dump(input_lang.word2index, f)

    with open("output_vocab_dict.json", "w") as f:
          json.dump(output_lang.word2index, f)

    print("Vocabs Saved")
    dict_count = list(input_lang.word2index.keys())
    print('TIL:',len(dict_count))
    gen_embed_input()
    gen_embed_output()
    print(train_pairs[0])
    # Initialize models
