...
class EncoderSeq(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5):
        ...
        with open('input_embeddings_dict.pickle', 'rb') as handle:
        ...
        #X_np_tsne = TSNE(n_components=128, method='exact').fit_transform(emp_emb)
        #print("Emb input shape", X_np_tsne.shape)
        x_torch = torch.from_numpy(emp_emb)
        x_torch = x_torch.type(torch.FloatTensor)
        self.embedding.weight = nn.Parameter(x_torch)

        for p in self.embedding.parameters():
            p.requires_grad = False

        print(" Input Embeddings Loaded")
        self.em_dropout = nn.Dropout(dropout)
...
class GenerateNode(nn.Module):
    def __init__(self, hidden_size, op_nums, embedding_size, dropout=0.5):
        ...
        with open('output_embeddings_dict.pickle', 'rb') as handle:
          embb = pickle.load(handle)
        ...
        #X_np_tsne = TSNE(n_components=128, method='exact').fit_transform(emp_emb)
        #print("Emb output shape", X_np_tsne.shape)
        x_torch = torch.from_numpy(emp_emb)
        x_torch = x_torch.type(torch.FloatTensor)
        self.embeddings.weight = nn.Parameter(x_torch)

        for p in self.embeddings.parameters():
            p.requires_grad = False
        print("Output Embeddings Loaded")
        self.em_dropout = nn.Dropout(dropout)
